...
const dsm=readAsc('alva_dsm_0p5m.asc');
const dtm=readAsc('alva_dtm_0p5m.asc');
...
const meta={
  ...
  baseFt:baseM*FT,
  bin:{file:'terrain.bin',layout:'Float32LE: DSM[nc*nr] then DTM[nc*nr]'},
...
writeFileSync('terrain_meta.json',JSON.stringify(meta,null,2));
...
console.log('wrote terrain.bin',(buf.length/1e6).toFixed(2),'MB + terrain_meta.json');
